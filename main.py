...
# PUBLIC_BASE_URL
# ----------------------------------------------------------------------------
# Group invite links are built from this. By default it's empty, which means
# the app falls back to whatever address is in the browser bar -- fine for
# testing on one phone, but useless if you send that link to someone else,
# because it will be your local IP (192.168.x.x) or 127.0.0.1, which only
# works on YOUR network.
#
# To make invite links open correctly for ANYONE, no matter where they are,
# this server needs a real public address, then set that address here (or
# via the HM_PUBLIC_URL environment variable). Two common ways to get one
# on Termux without owning a server:
#   1. Cloudflare Tunnel (free): `cloudflared tunnel --url http://localhost:5000`
#      gives you a stable https://xxxx.trycloudflare.com address.
#   2. ngrok (free tier): `ngrok http 5000` gives a temporary public https URL.
# For a permanent setup, deploy this app on a VPS with a domain name instead.
#
# Example: HM_PUBLIC_URL=https://your-tunnel-address.trycloudflare.com
...
def serialize_message(conn, r):
    ...
    if reply_to_id:
        orig = conn.execute("SELECT * FROM messages WHERE id=?", (reply_to_id,)).fetchone()
        if orig:
            ...
            reply_to = {
                ...
                "preview": "Message deleted" if orig["deleted"] else preview_text(orig),
                "msg_type": orig["msg_type"],
            }

    forwarded_from_name = None
    forwarded_from_id = r["forwarded_from_id"] if "forwarded_from_id" in r.keys() else None
    if forwarded_from_id:
        orig_user = conn.execute("SELECT username FROM users WHERE id=?", (forwarded_from_id,)).fetchone()
...
@auth_required
def delete_account():
    """Permanently erases this account and everything it authored. Once this
    completes, the email is free to register again, and nobody can message
    this account any more because it simply no longer exists."""
    data = request.get_json(force=True) or {}
    password = data.get("password") or ""
...
@auth_required
def conversations():
    """Unified, professional 'Chats' list: every direct conversation that has
    at least one message, plus every group you belong to, sorted by most
    recent activity -- exactly what was missing before, which is why a
    freshly started chat never showed up in the list."""
    db = get_db()
    uid = g.user["id"]
    convos = []
    ...
    for row in dm_chat_ids:
        ...
        convos.append({
            ...
            "avatar": partner["avatar"], "status": partner["status"],
    ...
    for grp in group_rows:
        ...
        convos.append({
            "type": "group", "id": grp["id"], "name": grp["name"], "avatar": grp["avatar"],
            "status": "", "last_preview": preview, "last_timestamp": ts, "last_is_mine": is_mine,
            "role": grp["role"], "invite_token": grp["invite_token"],
            "has_password": bool(grp["password_hash"]), "unread": unread,
        })
...
@app.route("/api/read/<chat_type>/<chat_id>", methods=["POST"])
@auth_required
def mark_read(chat_type, chat_id):
    db = get_db()
    last = db.execute(
    ...
    db.execute(
        ...
        (g.user["id"], chat_type, chat_id, last_id),
    )
    db.commit()
    return jsonify({"ok": True})
...
def sio_send_message(data):
    ...
    # A reply must point at a real, non-deleted message in the SAME chat.
    if reply_to_id:
        orig = conn.execute(
...
def sio_forward_message(data):
    ...
    chat_type = (data or {}).get("chat_type")   # target chat
...
def hide_message(msg_id):
    """Delete for me -- hides the message only for the current user, on every
    device they use, without affecting anyone else's copy of it."""
    db = get_db()
    db.execute(
...
def sio_edit_message(data):
    ...
    if not new_content:
        emit("error_msg", {"error": "Message can't be empty"})
        return
    conn = db_conn()
    row = conn.execute("SELECT * FROM messages WHERE id=?", (msg_id,)).fetchone()
...
# --------------------------------------------------------------------------
# Calls (WebRTC signaling relay -- 1:1 audio/video, DMs only)
# --------------------------------------------------------------------------
# The server never touches media; it only relays the small JSON offer/answer/
# ICE-candidate messages between the two participants' personal rooms so
# their browsers can negotiate a direct (or STUN-assisted) peer connection.
...
# --------------------------------------------------------------------------
# Reels (short video/photo posts with views, reactions, threaded comments)
# --------------------------------------------------------------------------
...
def serialize_comment(db, c):
    ...
    return {
        "id": c["id"], "reel_id": c["reel_id"], "user_id": c["user_id"],
        "author_name": author["username"] if author else "Unknown",
        "author_avatar": author["avatar"] if author else "",
...
def try_start_public_tunnel():
    """Best-effort attempt to give this server a real internet address, so
    group invite links work for anyone, anywhere -- not just on the same
    local network. Uses `cloudflared`'s free, no-signup "quick tunnel".
    If cloudflared isn't installed, this quietly does nothing and the app
    falls back to warning the admin (see PUBLIC_BASE_URL notes above)."""
    global PUBLIC_BASE_URL
    if PUBLIC_BASE_URL:
        print(f"Using configured public URL for invite links: {PUBLIC_BASE_URL}")
        return
    ...
    def runner():
        ...
        try:
            ...
            print(
                "\nNote: group invite links currently only work on your local network.\n"
                "To make them work over the internet automatically, install cloudflared:\n"
                "    pkg install cloudflared          (Termux)\n"
                "    apt install cloudflared          (Debian/Ubuntu)\n"
                "then just restart hm.py -- no account or signup needed.\n"
                "(Alternatively set the HM_PUBLIC_URL environment variable yourself.)\n"
            )
            return
        ...
        for line in proc.stdout:
            ...
            if m:
                ...
                print(f"\n✔ Group invite links now work over the internet: {PUBLIC_BASE_URL}\n")
                break
